...
--// GITHUB LOADER (EXECUTOR ENHANCED)
local HttpService = game:GetService("HttpService")
local GITHUB_BASE = "https://raw.githubusercontent.com/cosmicsys/netf/refs/heads/main/"

-- Use getgenv() if available for persistent global state
local env = (getgenv and getgenv()) or _G

local function loadModule(path)
    print("Loading module: " .. path)
    local success, response = pcall(function()
        return HttpService:GetAsync(GITHUB_BASE .. path)
    end)
...
Error: " .. tostring(response))
    ...
    if not fn then
        warn("Failed to compile module: " .. path .. "
Error: " .. tostring(err))
        return nil
    end

    -- Run in the protected environment
...
-- Export loader to global environment
